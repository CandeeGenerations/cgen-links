...
export abstract class IQuery {
    abstract findLinksByOwner(owner: string, _size?: number, _cursor?: string): QueryFindLinksByOwnerPage | Promise<QueryFindLinksByOwnerPage>;

    abstract findAuthorizedUser(googleId: string, authorized: boolean): User | Promise<User>;

    abstract findActiveLinksByOwner(owner: string, _size?: number, _cursor?: string): QueryFindActiveLinksByOwnerPage | Promise<QueryFindActiveLinksByOwnerPage>;

    abstract findUserByGoogleId(googleId: string): User | Promise<User>;

    abstract findLinksByTitle(owner: string, title: string, _size?: number, _cursor?: string): QueryFindLinksByTitlePage | Promise<QueryFindLinksByTitlePage>;

    abstract findLinkByID(id: string): Link | Promise<Link>;

    abstract findUserByID(id: string): User | Promise<User>;

    abstract findSettingsBySlug(slug: string): Settings | Promise<Settings>;

    abstract findClickByID(id: string): Click | Promise<Click>;

    abstract clickCountByOwner(owner: string): number | Promise<number>;

    abstract findClicksByOwner(owner: string, _size?: number, _cursor?: string): QueryFindClicksByOwnerPage | Promise<QueryFindClicksByOwnerPage>;

    abstract findSettingsByID(id: string): Settings | Promise<Settings>;

    abstract findLinksByDestination(owner: string, destination: string, _size?: number, _cursor?: string): QueryFindLinksByDestinationPage | Promise<QueryFindLinksByDestinationPage>;
}

export class QueryFindActiveLinksByOwnerPage {
    data: Link[];
    after?: string;
    before?: string;
}
